    ...
    const {errors, onSubmit, control, getValues, setValue, watch, reset} = useZod<AddNewAdForm>(
        {
            ...
            photos: z.array(
                zPhoto(true, "ads.additional_ad_images", t)
...
function CategoriesSection({t, locale, control, getValues, setValue, errors, watch, isLoading, categories}) {
    ...
    useEffect(() => {
        ...
        set_current_sub_categories(
            selected_category.sub_categories.map(
                (e) => ({
                    id: e.id,
                    name: e.names[locale!],
                    value: e.id,
...
function FiltersChoicesSection({t, locale, control, getValues, setValue, watch, isLoading, categories}) {
    ...
    const on_change = (id: string, index: number, value: string) => {
        ...
        if (
            next_filter &&
            new_selected_items.findIndex((e) => e.id === next_filter.id) === -1
        ) {
            next_filter.value = "";
        ...
        found_index = current_filters_choices.findIndex(
            (choice) => choice.id === id
        );
    ...
    return (
        current_filters_choices.length > 0 && <section className="space-y-8">
            ...
            {selected_items.map((item, index) => {
                ...
                const values = current_options_list.length !== 0 ? current_options_list[original_index] : filter.values;
