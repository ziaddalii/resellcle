...
function build_form_data(default_values: Props["default_values"], validated_data: EditAdForm) {
    ...
    // DELETE PHOTOS
    validated_data.photos_delete?.forEach((e, i) => {
        form_data.append(`photos[${i + photos_add_length + photos_update_length}][id]`, e);
    });
    ...
    const deleted_descriptors = default_values.descriptors.filter(
        (e) => validated_data.descriptors?.findIndex((f) => f.id === e.id) === -1
    );
    ...
    const updated_descriptors = validated_data.descriptors?.filter(
        (e) => default_values.descriptors?.findIndex((f) => f.id === e.id) !== -1
    );
    updated_descriptors.forEach((e, i) => {
        const found_index = default_values.descriptors?.findIndex((f) => f.id === e.id);

        if (e.value !== default_values.descriptors[found_index].value) {
    ...
    new_descriptors.forEach((e, i) => {
        ...
        form_data.append(
            `descriptors[${updated_descriptors.length + deleted_descriptors.length + i}][descriptor_id]`,
            e.id
        );
...
export default function EditAdForm({
    ...
    return (
        <Box component={"form"} onSubmit={onSubmit} className="space-y-8">
            ...
            {/*Photos*/}
            <PhotosSection t={t} errors={errors} isLoading={isLoading} getValues={getValues} setValue={setValue} />
...
function LocationSection({ t, control, getValues, watch, setValue, errors, isLoading, countries, provinces, cities }) {
    const [provinces_list, set_provinces_list] = useState<ProvinceData[]>(
        provinces.filter((province) => province.country?.id === getValues("region"))
    );

    const [cities_list, set_cities_list] = useState<CityData[]>(
        cities.filter((city) => city.province?.id === getValues("province"))
    );
...
function CategoriesSection({ t, locale, control, getValues, setValue, errors, watch, isLoading, categories }) {
    ...
    useEffect(() => {
        ...
        // SUB_CATEGORIES
        const new_sub_categories = selected_category.sub_categories.map((e) => ({
            id: e.id,
            name: e.names[locale!],
            value: e.id,
        }));
        ...
        if (new_sub_categories.findIndex((e) => e.id === getValues("sub_category")) === -1) {
    ...
    return (
        <>
            <section className="space-y-8">
                ...
                {/* Sub Categories*/}
                {current_sub_categories.length > 0 && (
                    <Controller
                        name="sub_category"
                        control={control}
                        render={({ field }) => (
