...
export default async function TopFooterSection({regions, categories, locale, t}: Props) {
    ...
    return (
            <Box className="bg-secondary-100 p-4">
                <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-4 justify-between">
                    ...
                    <FooterGroup
                            single_column={false}
                            title={t!("fields.categories")} icon={<Category/>}
                            items={categories.map((e: CategoryModel) => ({
                                name: e.names[locale],
                                link: e.super_category?.id ?
                                        `/categories/${encodeURIComponent(e.super_category.names.en)}/${encodeURIComponent(e.names.en)}` :
                    ...
                    {my_account_data.map((e, i) => (
                            <FooterGroup
                                    key={i}
                                    title={t!(e.title)}
                                    items={e.items.map((f) => ({...f, name: t!(f.name)}))}
                            />
                    ))}
...
function FooterGroup({title, icon, items, single_column = true}: FooterGroupProps) {
    ...
    const list: ReactNode = single_column ?
            ...
            </ul>) : (<ul className="text-black gap-2 px-2 mt-2 grid grid-cols-2">
                {items.map((item, i) => (
                        <li key={i} className="flex gap-1 link-primary">
                            {icon && icon}
                            {item.icon && item.icon}
