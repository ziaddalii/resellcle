...
export async function generateMetadata({params: {locale, category, page}}: Props): Promise<Metadata> {
    
    const t = await getTranslator(locale);
    
    const parsed_page = format_page_num(page);
    
    const {category: cat} = await get_category(category, parsed_page);
    ...
    return {
        ...
        openGraph: {
            ...
            description: cat.name,
            url: `https://resellcle.com/${locale}/categories/${encodeURIComponent(category)}?page=${parsed_page}`,
            locale,
            images: {
                url: cat.icon_url,
                alt: cat.name,
...
export default async function CategoryPage(props: Props) {
    ...
    const current_base_url = `/categories/${encodeURIComponent(category)}?${build_query_params(props.searchParams)}`;
    ...
    return (
        <Box component="main">
            ...
            {/*Nav Header*/}
            <HeaderNavButtons
                items={[{name: response_data.category.names[locale], link: `/categories/${category}?page=1`}]}/>
            ...
            <Container maxWidth="xl" component="section" className="space-y-8">
                ...
                <PaginationBar
                    ...
                    query_builder={(p) => build_query_params({
                        ...({sort_by, sub_category, ...other_query_filters}),
                        page: p,
                    })}
